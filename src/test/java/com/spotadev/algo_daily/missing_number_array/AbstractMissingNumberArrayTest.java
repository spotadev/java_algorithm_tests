/*
    ================================================================================================
    This code is part of java_algorith_tests which is an effort of SpotADev

    java_algorith_tests is used for getting SpotADev devs up to speed so they can pass tests on
    java algorithms.
    
    Copyright (C) 2021 java_algorith_tests

    ================================================================================================
    Author : John Dickerson
    ================================================================================================
*/
package com.spotadev.algo_daily.missing_number_array;

import java.util.List;

import org.testng.Assert;

/**
 * @author John Dickerson - 3 Jan 2023
 */
public abstract class AbstractMissingNumberArrayTest {

    protected MissingNumberArrayAPI missingNumberArray;

    public void missingNumber_1Test() {

        int[] numbers = { 1, 2, 4, 5, 7 };
        List<Integer> missingNumbers = missingNumberArray.missingNumber( numbers );
        Assert.assertEquals( missingNumbers.size(), 2 );
        Assert.assertEquals( missingNumbers.get( 0 ).intValue(), 3 );
        Assert.assertEquals( missingNumbers.get( 1 ).intValue(), 6 );
    }
}
